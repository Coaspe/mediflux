...
import { PRecord, QueryDataName, TableType, User } from "~/type";
        ...
        sx: {
          ...
          pointerEvents:
            row.original.LockingUser && row.original.LockingUser?.id != user.id
              ...
              : "default",
          height: `${
            density === "compact" ? 45 : density === "comfortable" ? 50 : 57
          }px`,
        ...
        queryClient.setQueryData([queryDataName], (prevs: any) => {
          ...
          prevs?.forEach((prevPRecord: PRecord) => {
            if (prevPRecord.id !== newPRecord.id) {
              ...
              !isInvalidOpReadiessWithTable(newPRecord, queryDataName)
            ) {
